...
__global__ void gemm_v4_kernel(float *__restrict__ A, float *__restrict__ B,
...
#pragma unroll
    ...
    for (int k = 0; k < BLOCK_SIZE_K; k++) {
      for (int i = 0; i < TILE_SIZE_M; i += 4) {
        ...
        a[i + 1] = As[ty * TILE_SIZE_M + i + 1][k];
        a[i + 2] = As[ty * TILE_SIZE_M + i + 2][k];
        a[i + 3] = As[ty * TILE_SIZE_M + i + 3][k];
        FETCH_FLOAT4(b[i]) = FETCH_FLOAT4(Bs[k][tx * TILE_SIZE_N + i]);
      }
...
template <int TILE_SIZE_M, int TILE_SIZE_N, int BLOCK_SIZE_M, int BLOCK_SIZE_N,
          int BLOCK_SIZE_K, int M, int N, int K>
void gemm_v4(const float *A, const float *B, float *C) {
  ...
  // Launch kernel
  gemm_v4_kernel<TILE_SIZE_M, TILE_SIZE_N, BLOCK_SIZE_M, BLOCK_SIZE_N,
                 BLOCK_SIZE_K, M, N, K>
